...
export default function StickmanFigure({ row, x, y, showLabel, status }: Props) {
  ...
  return (
    ...
    >
      {showLabel && (
        <div className="absolute -top-12 left-1/2 -translate-x-1/2 text-center pointer-events-none whitespace-nowrap">
          <div className="text-[10px] text-ink-300 font-mono">#{row.rank}</div>
          <div className="text-[12px] font-bold leading-tight" style={{ color: colour }}>
            {row.username}
          </div>
          <div className="text-[10px] text-ink-300/80">{row.student_id}</div>
        </div>
      )}
      ...
      {/* The figure itself: a stylised matchstick */}
      <motion.svg
        viewBox="0 0 56 84"
        width={FIGURE_W}
        height={84}
